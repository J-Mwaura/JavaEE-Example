 ...
 */
@WebServlet(urlPatterns = "/image/*") // annotation for listening to images
public class ImageServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        ArticleTable p = articleFacade.find(Integer.parseInt(requestedImage));

        if ((p == null) || (p.getImageMain()== null)) {
            response.sendError(HttpServletResponse.SC_NOT_FOUND); // 404.
        } else {
            /*
            ...
            response.setHeader("Content-Length", String.valueOf(p.getImageMain().length));
            response.setHeader("Content-Disposition", "inline; filename=\"" + p.getArticleTitle()+ "\"");
            ...
            try {
                // Open streams.
                byteInputStream = new ByteArrayInputStream(p.getImageMain());
